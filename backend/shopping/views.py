...
from family.views import get_familia
...
def lista_compras(request):
    ...
    if request.method == 'GET':
        itens = ItemListaCompra.objects.filter(familia=_familia)
...
def pedidos_compra(request):
    ...
    if request.method == 'GET':
        pedidos = PedidoCompra.objects.filter(familia=_familia).prefetch_related('itens').order_by('-pedido_em')
